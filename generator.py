...
class BatchGenerator(Sequence):
    def __init__(self,
        ...
        self.min_net_size       = (min_net_size//self.downsample)*self.downsample   # secures that multiple of downsample
        self.max_net_size       = (max_net_size//self.downsample)*self.downsample   # secures that multiple of downsample
    ...
    # number of batches in the Sequence = Dataset
    def __len__(self):
        return int(np.ceil(float(len(self.instances))/self.batch_size))

    # gets batch at Sequence position idx
    def __getitem__(self, idx):
        # get image input size, change "in average" every 10 batches due to fact
        # in training we shuffle, therefore idx will not be ordered!
    ...
    def _get_net_size(self, idx):
        if idx%10 == 0:
            #randint(start,end) generates random integer [start,end]
