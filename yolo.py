...
class YoloLayer(Layer):
    # Writting own keras layers see https://keras.io/layers/writing-your-own-keras-layers/
    ...
    def call(self, x):
        ...
        # adjust the shape of the y_predict [batch, grid_h, grid_w, 3 anchors, 4+1+nb_class]
        ...
        grid_h      = tf.shape(y_true)[1]   # grid height
        grid_w      = tf.shape(y_true)[2]   # grid width
        grid_factor = tf.reshape(tf.cast([grid_w, grid_h], tf.float32), [1,1,1,1,2])

        net_h       = tf.shape(input_image)[1]  # network (scaled image) height
        net_w       = tf.shape(input_image)[2]  # network (scaled image) width
        net_factor  = tf.reshape(tf.cast([net_w, net_h], tf.float32), [1,1,1,1,2])

        """
        Adjust prediction
        """
        # extract the 1+2 / 3+4 / 5 / all other dimension of the last dimension of size 4+1+nb_class
        # calculate the bounding box parameters from the predicted values
        ...
        tuning_loss = True
        ######################################################
        if tuning_loss is True:
            # commented by Andreas, otherwise alot of prints in log-file
